/* ═══════════════════════════════════════════════════════════════
   Dithered swirl background — WebGL2, no dependencies.
   Ported from a React/paper-shaders component (Originkit) to plain
   JS for this static site (no build step, no React).
   ═══════════════════════════════════════════════════════════════ */
...
class ShaderMountVanilla {
  constructor(parentElement, fragmentShader, uniforms, speed = 0) {
    ...
    this.program = createProgram(gl, vertexShaderSource, fragmentShader);
    if (!this.program) throw new Error('Failed to build shader program');
    ...
    this._onVisibility = () => this.setCurrentSpeed(document.hidden ? 0 : this.speed);
    document.addEventListener('visibilitychange', this._onVisibility);
  }

  handleResize() {
    const dpr = Math.min(window.devicePixelRatio || 1, 2);
    const width = Math.round((this.parentElement.clientWidth || window.innerWidth) * dpr);
    const height = Math.round((this.parentElement.clientHeight || window.innerHeight) * dpr);
...
/**
 * initDitherBackground(container, options) → { destroy() }
 * options: { background, color, size, speed, scale, shape, type, fit, offsetX, offsetY }
 */
export function initDitherBackground(container, options = {}) {
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  const background = options.background ?? '#FFFFFF';
  const color = options.color ?? '#8AAFCF';
  const shape = DitheringShapes[options.shape ?? 'swirl'];
  const type = DitheringTypes[options.type ?? '4x4'];
  const fit = ShaderFitOptions[options.fit ?? 'cover'];
  const size = options.size ?? 3.4;
  const speed = reducedMotion ? 0 : (options.speed ?? 1.0);
  const scale = options.scale ?? 1.32;
  ...
  let mount = null;
  try {
    mount = new ShaderMountVanilla(container, ditheringFragmentShader, uniforms, speed);
  } catch (e) {
    console.warn('Dither background skipped:', e.message);
    return { destroy() {} };
  }
  ...
  if ('IntersectionObserver' in window) {
    io = new IntersectionObserver(([entry]) => {
      mount.setCurrentSpeed(entry.isIntersecting ? speed : 0);
    }, { threshold: 0 });
